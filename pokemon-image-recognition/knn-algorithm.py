...
def run_test(k):
    ...
    if (chr(k) == 'a'):
        # print("\nComputing Small Test Set")
        data_path = "Models/knn-data-1.txt"
    elif (chr(k) == 'b'):
        # print("\nComputing Gen 1 Set")
        data_path = "Models/knn-data-2.txt"
    elif (chr(k) == 'c'):
        # print("\nComputing Complete Test Set")
        data_path = "Models/knn-data-3.txt"
    else:
        # Case when this is a number. This is when we're NOT getting the analytics.
        # Default to test on small test set:
        data_path = "Models/knn-data-1.txt"

    # When running analytics, automatically switch k to be 4 for the following tests. Somehow, this is an int...
    if (type(k) is int):
        k = 4
...
def main():
    ...
    if (userInput == 1):
        ...
        for code in range(ord('a'), ord('c') + 1):
            accuracy, process_time = run_test(code)
        ...
        # Set the x-ticks.
        ax1.set_xticklabels(["6 Pokemon", "151 Pokemon (Gen 1)", "721 Pokemon (Gen 1-6)"], rotation=0.1)
        # Create a title.
